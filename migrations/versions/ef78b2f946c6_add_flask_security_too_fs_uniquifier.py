...
def upgrade():
    op.add_column('users', sa.Column('fs_uniquifier', sa.String(length=64), server_default=sa.text("gen_random_uuid()"), nullable=False))
    op.create_unique_constraint(op.f('uq_users_fs_uniquifier'), 'users', ['fs_uniquifier'])
    op.add_column('users_version', sa.Column('fs_uniquifier', sa.String(length=64), autoincrement=False, nullable=True))
