"""add project_id and organization_id to audit_log view

Revision ID: d5c2e2c0768a
Revises: 1f913a84ab35
Create Date: 2026-03-31 13:02:22.628108
...
SELECT a.id, t.issued_at AS timestamp,
  ...
  cfg_app.project_id, cfg_proj.organization_id,
...
SELECT a.id, t.issued_at,
  ...
  img_app.project_id, img_proj.organization_id,
...
SELECT a.id, t.issued_at,
  ...
  rel_app.project_id, rel_proj.organization_id,
  COALESCE(u.username, tx_u.username), COALESCE(u.email, tx_u.email), t.remote_addr,
  NULL, NULL, NULL,
  NULL, NULL,
  NULL,
...
SELECT a.id, t.issued_at,
  ...
  dep_app.project_id, dep_proj.organization_id,
  COALESCE(u.username, tx_u.username), COALESCE(u.email, tx_u.email), t.remote_addr,
  NULL, NULL, NULL,
  NULL, NULL,
  dep_rel.version,
...
SELECT a.id, t.issued_at,
  ...
  ing_app.project_id, ing_proj.organization_id,
  COALESCE(u.username, tx_u.username), COALESCE(u.email, tx_u.email), t.remote_addr,
  NULL, NULL, NULL,
  NULL, NULL,
  NULL,
...
SELECT a.id, t.issued_at,
  ...
  app.id, NULL::uuid,
  app.project_id, proj.organization_id,
  COALESCE(u.username, tx_u.username), COALESCE(u.email, tx_u.email), t.remote_addr,
  NULL, NULL, NULL,
  NULL, NULL,
  NULL,
...
def downgrade():
    # Revert to previous version without project_id/organization_id
    # The previous migration (14392342a190) will recreate the old view
    op.execute(sa.text("DROP VIEW IF EXISTS audit_log"))
