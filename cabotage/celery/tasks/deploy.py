...
def _fargate_preview_namespace():
    """Return the configured Fargate preview namespace, or None."""
    return current_app.config.get("FARGATE_PREVIEW_NAMESPACE")
...
def k8s_namespace(release):
    ...
    if env and getattr(env, "ephemeral", False):
        fargate_ns = _fargate_preview_namespace()
        if fargate_ns:
            return fargate_ns
