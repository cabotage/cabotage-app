...
def process_deployment_hook(hook):
    ...
    pending_status_request = requests.post(
        ...
        json={'state': 'pending', 'description': 'Deployment is starting!'},
    )
    ...
    success_status_request = requests.post(
        deployment['statuses_url'],
        headers={
            'Accept': 'application/vnd.github.ant-man-preview+json',
            'Authorization': f'token {access_token["token"]}',
        },
        json={'state': 'pending', 'description': 'Code retrieved! Image build commencing'},
    )
    return True
